...
class NSSolver:
    ...
    def advance_one_step(self,t):        
        ...
        F_v = inner(nabla_grad(v_new),nabla_grad(y))*dx +\
          ...
          inner(phi_old*outer(p_old,p_old),nabla_grad(y))*dx 
        ...
        F_phider = phider_new*w2*dx - (a/(2*phicr**4))*phi_new*(phi_new-phi0)*(2*phi_new-phi0)*w2*dx -\
...
#Rescale velocities for the correct analysis    
v_means = np.zeros(numSteps)
