...
for i in tqdm(range(numSteps)):
    ...
    try:
        ...
        sumstat[i, 1] = 100 * w_sa * assemble((abs(dot(p_old,field)) * p_old[0])* dx_sub(1)) / area
    ...
    try:
        ...
        sumstat[i, 3] = 100 * w_sa * assemble((abs(dot(p_old,field)) * p_old[0])* dx_sub(1)) / area
    except Exception as e:
        print('trailing', i, e)
    ...
    try:
        ...
        top_pol = 100 * w_sa * assemble((abs(dot(p_old,field)) * p_old[0])* dx_sub(1)) / area
    except Exception as e:
        print('top', i, e)
    ...
    try:
        ...
        bottom_pol = 100 * w_sa * assemble((abs(dot(p_old,field)) * p_old[0])* dx_sub(1)) / area
    except Exception as e:
        print('bottom', i, e)
...
assemble((abs(dot(p_old,E)) * E[0])* dx_sub(1))


#(1+dot(p_old,field)) * p[0]
