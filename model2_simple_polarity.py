...
class edge(UserExpression):
    def eval(self, value, x):
        value[:] = 0
        if (10 < x[0] < 20 and 45 < x[1] < 47) or (10 < x[0] < 20 and 53 < x[1] < 55) or (
                10 < x[0] < 12 and 45 < x[1] < 55) or (18 < x[0] < 20 and 45 < x[1] < 55):
            value[:] = 1
...
bulk_file = File('results/bulk.pvd', 'compressed')
...
for i in tqdm(range(numSteps)):
    ...
    v_old.rename('v', 'v')
    p_old.rename('p', 'p')
    bulk_region_old.rename('bulk', 'bulk')

    phi_file << phi_old
    v_file << v_old
    p_file << p_old
    bulk_file << bulk_region_old

    # Compute leading edge outgrowth
    cf = MeshFunction("size_t", mesh, mesh.topology().dim(), 0)
    region = AutoSubDomain(lambda x, on: leading_edge_old(x) > thres)
    region.mark(cf, 1)
    dx_sub = Measure('dx', subdomain_data=cf)
    area = assemble(E[0] * dx_sub(1))
    try:
        sumstat[i, 0] = U * assemble(v_old[0] * dx_sub(1)) / area
        sumstat[i, 1] = 100 * w_sa * assemble(p_old[0] * dx_sub(1)) / area
    except Exception as e:
        print('leading', i, e)

    # Compute trailing edge outgrowth
    cf = MeshFunction("size_t", mesh, mesh.topology().dim(), 0)
    region = AutoSubDomain(lambda x, on: trailing_edge_old(x) > thres)
    region.mark(cf, 1)
    dx_sub = Measure('dx', subdomain_data=cf)
    area = assemble(E[0] * dx_sub(1))
    try:
    ...
    cf = MeshFunction("size_t", mesh, mesh.topology().dim(), 0)
    region = AutoSubDomain(lambda x, on: top_edge_old(x) > thres)
    region.mark(cf, 1)
    dx_sub = Measure('dx', subdomain_data=cf)
    area = assemble(E[0] * dx_sub(1))
    try:
        top_vel = U * assemble(v_old[0] * dx_sub(1)) / area
        top_pol = 100 * w_sa * assemble(p_old[0] * dx_sub(1)) / area
    except Exception as e:
        print('top', i, e)

    # Compute bottom zone speed
    cf = MeshFunction("size_t", mesh, mesh.topology().dim(), 0)
    region = AutoSubDomain(lambda x, on: bottom_edge_old(x) > thres)
    region.mark(cf, 1)
    dx_sub = Measure('dx', subdomain_data=cf)
    area = assemble(E[0] * dx_sub(1))
    try:
        bottom_vel = U * assemble(v_old[0] * dx_sub(1)) / area
        bottom_pol = 100 * w_sa * assemble(p_old[0] * dx_sub(1)) / area
    except Exception as e:
        print('bottom', i, e)
...
np.savetxt('model2_thresholds/' + 'model2_thres' + str(thres) + '.txt', sumstat)
