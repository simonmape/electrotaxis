...
for sumstat in tqdm(os.listdir('Model2_Outputs/')):
    if re.search(r'\d', sumstat):
        #Read in parameter values used from file name
        reduced = sumstat.replace('_dph','*').replace('model2_cE','').replace('.txt','')
        reduced = reduced.replace('_','.')
        params = reduced.split('*')
        cE = float(params[0])
        delta_ph = float(params[1])
        #Read the summary statistic
        summary = np.loadtxt('Model2_Outputs/'+sumstat)
        ...
        likelihoods.append([cE,delta_ph,le, te, tbe, bd, bs, ls])

df = pd.DataFrame(likelihoods, columns = ['cE','delta_ph','le', 'te', 'tbe', 'bs', 'bd', 'ls'])
df.to_csv('model2_loglikelihoods.csv')